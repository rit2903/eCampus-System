...
//    // List<CourseRequirement> findByProgramIdAndSchemeId(Long programId, Long schemeId);
    ...
    @Query(value = "SELECT DISTINCT ct.ctpcode AS code, ct.ctpname AS name, ct.ctpid as ctpid FROM ec2.coursetypes ct WHERE ct.crscat='ELECTIVE' AND ct.ctpid IN (SELECT DISTINCT sc.ctpid FROM ec2.schemecourses sc WHERE sc.scheme_id = :scheme_id AND (sc.splid = :splid OR sc.splid = 0) AND sc.sem_no <= :sem_no AND sc.crsid IS NULL)", nativeQuery = true)
