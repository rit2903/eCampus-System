...
public interface TermCourseAvailableForRepository extends JpaRepository<TermCourseAvailableFor, Long> {
    ...
    @Query(value = "SELECT * FROM ec2.termcourseavailablefor tca WHERE tca.tcabchid = :tcabchid AND tca.tcatcrid IN (SELECT tc.tcrid FROM ec2.termcourses tc WHERE tc.iscore = 'N' AND tc.tcrtrmid = :tcrtrmid)", nativeQuery = true)
